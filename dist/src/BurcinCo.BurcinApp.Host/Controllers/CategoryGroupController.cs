...
namespace BurcinCo.BurcinApp.Host.Controllers
{
...
#endif
    {
        ...
        {
            return Ok(_dbContext.CategoryGroups.AsQueryable());
        }
#else
...
#if (OData)
		[HttpPut]
	        public IActionResult Put([FromRoute] long key, [FromBody] Delta<CategoryGroup> delta)
#else
		[HttpPut("{key}")]
...
#if (OData)
        [HttpPatch]
	        public IActionResult Patch([FromRoute] long key, [FromBody] Delta<CategoryGroup> delta)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
