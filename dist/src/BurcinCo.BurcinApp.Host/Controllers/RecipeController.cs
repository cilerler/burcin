...
namespace BurcinCo.BurcinApp.Host.Controllers
{
...
#endif
    {
        ...
        {
            return Ok(_dbContext.Recipes.AsQueryable());
        }
#else
...
#if (OData)
        [HttpPut]
	        public IActionResult Put([FromRoute] long key, [FromBody] Delta<Recipe> delta)
#else
		[HttpPut("{key}")]
...
#if (OData)
        [HttpPatch]
	        public IActionResult Patch([FromRoute] long key, [FromBody] Delta<Recipe> delta)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
