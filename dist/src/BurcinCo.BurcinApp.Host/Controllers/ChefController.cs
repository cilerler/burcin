...
namespace BurcinCo.BurcinApp.Host.Controllers
{
...
#endif
    {
        ...
        {
            return Ok(_dbContext.Chefs.AsQueryable());
        }
#else
...
#if (OData)
		[HttpPut]
	        public IActionResult Put([FromRoute] long key, [FromBody] Delta<Chef> delta)
#else
		[HttpPut("{key}")]
...
#if (OData)
        [HttpPatch]
	        public IActionResult Patch([FromRoute] long key, [FromBody] Delta<Chef> delta)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
