...
namespace Burcin.Host.Controllers
{
	...
	/// </summary>
	//[ControllerName(CategoryGroupControllerModelConfiguration.ControllerName)]
#if (OData)
    [ODataRoutePrefix(nameof(CategoryGroup))]
    public class CategoryGroupController : ODataController
#else
	[ApiController]
...
#endif
    {
        ...
        public CategoryGroupController(IServiceProvider serviceProvider, ILogger<CategoryGroupController> logger, BurcinDatabaseDbContext dbContext)
        {
        ...
        [ProducesResponseType(typeof(CategoryGroup), StatusCodes.Status201Created)]
        public IActionResult Post([FromBody] CategoryGroup record)
        {
            ...
            _dbContext.CategoryGroups.Add(record);
            SaveChanges();
...
#if (OData)
        //[ProducesResponseType(typeof(ODataValue<IEnumerable<CategoryGroup>>), StatusCodes.Status200OK)]
		[ODataRoute]
        public IActionResult Get(ODataQueryOptions<CategoryGroup> options)
        {
            ...
            var query = _dbContext.CategoryGroups.AsQueryable();
...
#else
		[ProducesResponseType(typeof(CategoryGroup[]), StatusCodes.Status200OK)]
		public IActionResult Get()
		{
			var records = _dbContext.CategoryGroups.ToList();
			return Ok(records);
		}
#endif
...
#endif
        {
	        var record = _dbContext.CategoryGroups.Where(r => r.Id == key).AsQueryable();
		...
		[HttpPut]
		[ODataRoute("({key})")]
        public IActionResult Put([FromODataUri] long key, [FromBody] Delta<CategoryGroup> delta)
#else
		[HttpPut("{key}")]
		public IActionResult Put([FromRoute] long key, [FromBody] CategoryGroup delta)
#endif
        {
            if (!ModelState.IsValid)
            {
            ...
            CategoryGroup record = _dbContext.CategoryGroups.SingleOrDefault(r => r.Id == key);
            if (record == null)
            {
                return NotFound();
            }
        ...
        [HttpPatch]
		[ODataRoute("({key})")]
        public IActionResult Patch([FromODataUri] long key, [FromBody] Delta<CategoryGroup> delta)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            var record = _dbContext.CategoryGroups.SingleOrDefault(r => r.Id == key);
            if (record == null)
            {
                return NotFound();
            }
...
#else
		[HttpPatch("{key}")]
		public IActionResult Patch([FromRoute] long key, [FromBody] CategoryGroup delta) => Put(key, delta);
#endif
...
#endif
        {
            ...
            if (suspendOnly)
            {
                // CategoryGroup does not have a disable property.
            } else {
                _logger.LogDebug("Deleting {id}", record.Id);
                _dbContext.CategoryGroups.Remove(record);
                SaveChanges();
