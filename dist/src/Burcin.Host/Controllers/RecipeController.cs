...
namespace Burcin.Host.Controllers
{
...
#endif
    {
        ...
        public RecipeController(IServiceProvider serviceProvider, ILogger<RecipeController> logger, BurcinDatabaseDbContext dbContext)
        {
        ...
        public IActionResult Post([FromBody] Recipe record)
        {
...
#if (OData)
        //[ProducesResponseType(typeof(ODataValue<IEnumerable<Recipe>>), StatusCodes.Status200OK)]
		[ODataRoute]
...
#endif
        {
            var record = _dbContext.Recipes.Where(r => r.Id == key).AsQueryable();
        ...
        [HttpPut]
		[ODataRoute("({key})")]
        public IActionResult Put([FromODataUri] long key, [FromBody] Delta<Recipe> delta)
#else
		[HttpPut("{key}")]
		public IActionResult Put([FromRoute] long key, [FromBody] Recipe delta)
#endif
        {
            if (!ModelState.IsValid)
            {
            ...
            Recipe record = _dbContext.Recipes.SingleOrDefault(r => r.Id == key);
            if (record == null)
            {
                return NotFound();
            }
			...
			record.Yield = delta.Yield;
			record.GramPerYield = delta.GramPerYield;
			record.CategoryCode = delta.CategoryCode;
#endif
            SaveChanges();
        ...
        [HttpPatch]
		[ODataRoute("({key})")]
        public IActionResult Patch([FromODataUri] long key, [FromBody] Delta<Recipe> delta)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            var record = _dbContext.Recipes.SingleOrDefault(r => r.Id == key);
            if (record == null)
            {
                return NotFound();
            }
...
#else
		[HttpPatch("{key}")]
		public IActionResult Patch([FromRoute] long key, [FromBody] Recipe delta) => Put(key, delta);
#endif
