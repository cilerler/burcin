...
internal sealed partial class WebhookService : IWebhookService
{
	...
	public async Task<WebhookPublishResult> PublishAsync(string path, string body, CancellationToken cancellationToken = default)
	{
		...
		// Routing key + exchange name follow Ruya's MessageQueue.RabbitMq convention: one topic exchange
		// per topic, with a catch-all (#) bound queue. The receiving subscriber's IMessageQueue.SubscribeAsync
		// auto-creates exchange="<topic>", queue="<topic>.queue", binding=#. We publish to that same exchange so
		// downstream Inbox subscribers in any module pick the message up.
		var routingKey = $"webhooks.{path.Replace('/', '.')}";
		var exchange = routingKey;

		// Wrap the supplier's raw body in a MessageEnvelope shape so Ruya's MessageQueue subscriber can
		// deserialize MessageEnvelope<TMessage>. Gateway is the translation layer between the supplier's
		// flat JSON and the internal envelope format.
		var payloadJson = string.IsNullOrWhiteSpace(body) ? "{}" : body;
		...
		{
			properties = new { content_type = "application/json", delivery_mode = 2 },
		...
		try
		{
			...
			var response = await client.PostAsync(
				$"/api/exchanges/{_settings.VHost}/{exchange}/publish",
				content,
