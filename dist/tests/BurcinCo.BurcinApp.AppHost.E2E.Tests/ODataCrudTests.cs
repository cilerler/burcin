...
/// <summary>
/// Aspire-orchestrated end-to-end OData / minimal-API smoke tests. Spins up the full distributed
/// application (MsSql + Redis + RabbitMQ + Host + Gateway) the same way the AppHost does in dev,
/// then exercises the public HTTP surface through the gateway endpoint to prove:
...
///   - The non-database Tag entity is indistinguishable at the HTTP layer from a DB-backed entity.
...
///
/// Lighter-weight HTTP-layer tests via <c>WebApplicationFactory</c> were tried and abandoned: the
/// Host's heavy AddDefaultServices wire-up (Redis distributed lock, SQL distributed cache, OTel
/// exporters, RabbitMQ health checks) doesn't survive being booted without real infrastructure.
/// Aspire owns the infrastructure for us — that's the pattern we lean on.
///
...
public sealed class ODataCrudTests
{
	...
	public async Task Chef_DbBackedCrud_RoundTripIncludingPatchSemantics()
	{
		...
		using var http = await CreateHostHttpClientAsync(app);
	...
	[TestMethod]
	public async Task Tag_NonDbBackedCrud_BehavesIdenticallyAtHttpLayer()
	{
		// Same operations against /odata/Tag (in-memory ConcurrentDictionary). The whole point of
		// having Tag in the template is to demonstrate that OData CRUD looks identical regardless
		// of backing store, so this test mirrors the Chef test above in shape.
		await using var app = await StartAppAsync();
		using var http = await CreateHostHttpClientAsync(app);

		using var post = await http.PostAsJsonAsync("/odata/Tag", new { Name = "smoke", Color = "#abcdef" });
		Assert.AreEqual(HttpStatusCode.Created, post.StatusCode, $"POST /odata/Tag expected 201. Body: {await post.Content.ReadAsStringAsync()}");
		var created = await post.Content.ReadFromJsonAsync<JsonElement>();
		var id = created.GetProperty("Id").GetInt64();
		Assert.IsTrue(id > 0);

		using var patch = await http.PatchAsync($"/odata/Tag/{id}", JsonContent.Create(new { Color = "#00ff00" }));
		Assert.IsTrue(patch.IsSuccessStatusCode);

		using var afterPatch = await http.GetAsync($"/odata/Tag/{id}");
		var patched = await afterPatch.Content.ReadFromJsonAsync<JsonElement>();
		Assert.AreEqual("smoke", patched.GetProperty("Name").GetString(), "Non-DB PATCH must also leave Name alone.");
		Assert.AreEqual("#00ff00", patched.GetProperty("Color").GetString());

		using var delete = await http.DeleteAsync($"/odata/Tag/{id}");
		Assert.AreEqual(HttpStatusCode.NoContent, delete.StatusCode);
	}
	...
	{
		// Minimal-API command endpoint /api/sourcing/quotes. This is a verb-shaped flow (kicks
		...
		using var response = await http.PostAsJsonAsync("/api/sourcing/quotes/", body);

		Assert.AreEqual(HttpStatusCode.Accepted, response.StatusCode,
			$"POST /api/sourcing/quotes expected 202. Got {(int)response.StatusCode}: {await response.Content.ReadAsStringAsync()}");
		...
		Assert.IsTrue(view.TryGetProperty("id", out _) || view.TryGetProperty("Id", out _),
			"Response body should contain an Id (case-sensitivity depends on server JSON config).");
	}
#endif
	...
	private static async Task<HttpClient> CreateHostHttpClientAsync(DistributedApplication app)
	{
